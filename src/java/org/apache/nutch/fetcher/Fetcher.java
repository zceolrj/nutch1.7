    					...
    					MapRunnable<Text, CrawlDatum, Text, NutchWritable> 
{
	...
	private class FetcherThread extends Thread 
	{
	    				...
	    				do 
	    				{
	    					...
	    					case ProtocolStatus.SUCCESS:        // got a page
	    								...
	    								pstatus.getMinorCode() == ParseStatus.SUCCESS_REDIRECT) 
	    						{
	    							...
	    							Text redirUrl = handleRedirect(fit.url, fit.datum, urlString, newUrl,
	    												refreshTime < Fetcher.PERM_REFRESH_TIME, Fetcher.CONTENT_REDIR);
	    							if (redirUrl != null) 
	    							{
	    ...
	    private Text handleRedirect(Text url, CrawlDatum datum, String urlString, String newUrl, boolean temp, String redirType)
		        	...
		        	if (!origHost.equals(newHost)) 
		        	{
		        		if (LOG.isDebugEnabled()) 
		        		{
		        			LOG.debug(" - ignoring redirect " + redirType + " from " +
		        					urlString + " to " + newUrl + " because external links are ignored");
		        		}
		        		return null;
		        	}
		        	} 
                        ...
                        Content content, ProtocolStatus pstatus, int status, int outlinkDepth) 
	    {
	    	...
	    	if (content != null) 
	    	{
		        ...
		        /* Note: Fetcher will only follow meta-redirects coming from the original URL. */
	    	...
	    	try 
	    	{
	    		...
	    		if (parseResult != null) 
	    		{
	    			for (Entry<Text, Parse> entry : parseResult) 
	    			{
	    				...
	    				// Calculate page signature. For non-parsing fetchers this will be done in ParseSegment
	    				...
	    				if (maxOutlinkDepth > 0 && outlinkDepth < maxOutlinkDepth) 
	    				{
	    					...
	    					// Calculate variable number of outlinks by depth using the divisor 
	    					// (outlinks = Math.floor(divisor / depth * num.links))
	    					...
	    					while(iter.hasNext() && outlinkCounter < maxOutlinkDepthNumLinks) 
	    					{
	    						...
	    						FetchItem fit = FetchItem.create(new Text(followUrl), 
	    								new CrawlDatum(CrawlDatum.STATUS_LINKED, interval), queueMode, outlinkDepth + 1);
	    						fetchQueues.addFetchItem(fit);
	...
	private void checkConfiguration() 
	{
	    ...
	    // agent name is the first value in the agents we advertise for robot rules parsing
	    ...
	    else 
	    {
	    	...
	    	// if the first one is not equal to our agent name, log fatal and throw an exception
